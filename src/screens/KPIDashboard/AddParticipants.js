...
const AddParticipants = ({ navigation, route }) => {
  ...
  // Check if product_lines are received from AddProductLines and update formData
  useEffect(() => {
    if (route.params?.product_lines) {
      handleFieldChange("productLines", route.params.product_lines);
    }
  }, [route.params?.product_lines]);
