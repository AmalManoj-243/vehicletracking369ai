...
const PaymentForm = ({ navigation, route }) => {
  ...
  // --- Fetch payment journals on mount ---
  useEffect(() => {
    const loadJournals = async () => {
      setIsLoading(true);
      try {
        const data = await fetchPaymentJournalsOdoo();
        if (data && data.length > 0) {
          setJournals(data.map((j) => ({ id: j.id, label: `${j.name} (${j.type})`, type: j.type })));
        }
      } catch (err) {
        console.warn('Failed to load payment journals:', err?.message);
      } finally {
        setIsLoading(false);
      }
    };
