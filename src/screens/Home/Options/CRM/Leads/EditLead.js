...
const EditLead = ({ navigation, route }) => {
    ...
    const fetchDetails = async (leadId) => {
        ...
        try {
            ...
            setFormData((prevFormData) => ({
                ...
                salesPerson: { id: detail?.sales_preson?.sales_preson_id || null, label: detail?.sales_preson?.sales_preson_name || '' },
                priority: { label: detail?.priority || '' },
                ...
                expectedClosingDate: detail?.expected_closing_date || null,
            }));
        } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        if (validateForm(fieldsToValidate)) {
            ...
            try {
                const response = await put("/updateLead", leadData);
                if (response.success) {
    ...
    return (
        <SafeAreaView>
            <NavigationHeader title="Edit Leads" onBackPress={() => navigation.goBack()} />
